...
export interface SplitTextProps {
  ...
  immediate?: boolean; // Bypass ScrollTrigger and animate immediately
  animationDelay?: number; // Delay before starting animation (in seconds)
  reverse?: boolean; // Reverse the animation
  reverseDelay?: number; // Delay before reversing (in seconds)
}
...
const SplitText: React.FC<SplitTextProps> = ({
  ...
  useEffect(() => {
    if (reverse && timelineRef.current) {
      if (reverseDelay > 0) {
        ...
        // Immediate reverse without delay
        timelineRef.current.timeScale(2).reverse();
      }
    }
    ...
    () => {
      ...
      const splitInstance = new GSAPSplitText(el, {
        ...
        onSplit: (self: GSAPSplitText) => {
          ...
          // Only add ScrollTrigger if not immediate mode
          if (!immediate) {
          ...
          timelineRef.current = tween as any;
          return tween;
        },
      });
