...
interface ImageData {
  ...
  scaleX?: number; // Width scale multiplier
  scaleY?: number; // Height scale multiplier
  width?: number;
  height?: number;
}
...
//   { src: "/assets/img3.jpg", yPos: 0.7, xOffset: 0, row: "top" },
//   { src: "/assets/img4.jpg", yPos: -0.6, xOffset: 0.05, row: "bottom" },
//   // Second group
//   { src: "/assets/img5.jpg", yPos: 0.35, xOffset: 0, row: "top" },
//   { src: "/assets/img6.jpg", yPos: -0.75, xOffset: 0.1, row: "bottom" },
//   { src: "/assets/img7.jpg", yPos: 0.55, xOffset: 0, row: "top" },
//   { src: "/assets/img8.jpg", yPos: -0.55, xOffset: 0.05, row: "bottom" },
//   // Third group
//   { src: "/assets/img9.jpg", yPos: 0.8, xOffset: 0, row: "top" },
//   { src: "/assets/img10.jpg", yPos: -0.35, xOffset: 0.1, row: "bottom" },
//   { src: "/assets/img11.jpg", yPos: 0.6, xOffset: 0, row: "top" },
//   { src: "/assets/img12.jpg", yPos: -0.65, xOffset: 0.05, row: "bottom" },
// ];

// Replace the defaultImages array with:
const defaultImages: ImageData[] = layoutExamples.masonryCustomCluster;
//main: layoutExamples.grid2Rows
...
const GalleryCanvas: React.FC<GalleryCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    // IMPORTANT:
    // Don't double-drive Lenis. A separate GSAP ticker loop + RAF loop causes
    // duplicated work per frame and can make route enter/exit animations jank,
    // especially in production builds.
    ...
    // Hide gallery initially, will fade in shortly after mount
    // Note: Component mount is delayed until after page entrance animation
    if (canvasRef.current) {
      gsap.set(canvasRef.current, { opacity: 0 });
      gsap.to(canvasRef.current, {
        opacity: 1,
        duration: 0.8,
        delay: 0.1, // Small delay for smooth appearance after mount
        ease: "power2.out",
      });
    }
    ...
    // ============================================
    // IMAGE CLICK ENLARGEMENT STATE
    // ============================================
    let isEnlarging = false;
    let enlargingMesh: THREE.Mesh | null = null;
    let enlargeStartTime = 0;
    const enlargeConfig = {
      rippleDuration: 0.8, // Duration of initial ripple effect
      scaleDuration: 1.2, // Duration of scale animation
      delay: 0.3, // Delay before starting enlargement
      finalScale: 8, // How much to scale the image
      rippleIntensity: 2.5, // Intensity of the ripple effect on click
    };
    ...
    // ============================================
    // IMAGE CLICK HANDLER - ENLARGEMENT ANIMATION
    // ============================================
    function onCanvasClick(event: MouseEvent) {
      if (isEnlarging) return; // Prevent multiple clicks during animation

      // Use raycaster to detect which image was clicked
      mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
      mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
      ...
      if (intersects.length > 0) {
        ...
        // Trigger ripple effect from click point
        const material = clickedMesh.material as THREE.ShaderMaterial;
        if (material.uniforms) {
          material.uniforms.uMouse.value.copy(clickedUV);
        }

        console.log("Image clicked - starting enlargement animation");

        // ============================================
        // CUSTOM REDIRECT LOGIC - ADD YOUR CODE HERE
        // ============================================
        // This is where you can add custom logic that executes when an image is clicked
        // For example:
        // - Get the image data: const imageData = clickedMesh.userData.imgData;
        // - Get the image index: const imageIndex = clickedMesh.userData.index;
        // - Store data for navigation: sessionStorage.setItem('selectedImage', imageData.src);

        // Example: Log the clicked image information
        const imageData = clickedMesh.userData.imgData;
        const imageIndex = clickedMesh.userData.index;
        console.log("Clicked image data:", { imageData, imageIndex });

        // After the animation completes, you can navigate to another page
        // The navigation will happen after enlargeConfig.delay + enlargeConfig.scaleDuration
        const totalAnimationTime =
          (enlargeConfig.delay + enlargeConfig.scaleDuration) * 1000;

        setTimeout(() => {
          // ============================================
          // NAVIGATION LOGIC - ADD YOUR REDIRECT CODE HERE
          // ============================================
          // This code executes after the enlargement animation completes
          // Example navigation:
          // const router = (await import("next/router")).default;
          // router.push(`/project/${imageIndex}`);
          // or
          // window.location.href = `/project/${imageIndex}`;

          console.log("Animation complete - ready to navigate");

          // UNCOMMENT AND MODIFY THE LINE BELOW TO ENABLE NAVIGATION:
          // window.location.href = `/projects?id=${imageIndex}`;
        }, totalAnimationTime);
      }
    }
    ...
    images.forEach((img, index) => {
      textureLoader.load(
        ...
        (texture: THREE.Texture) => {
          ...
          // Support non-uniform scaling like the updated component
        ...
        gsap.fromTo(
          ...
          { x: 0 },
          {
            x: () => {
              ...
              const galleryMidpoint = totalWidth * 60; // 50% of totalWidth * 120
    ...
    function animate() {
      ...
      // ============================================
      // ENLARGEMENT ANIMATION LOGIC
      // ============================================
      if (isEnlarging && enlargingMesh) {
        const elapsedTime = (performance.now() - enlargeStartTime) / 1000; // Convert to seconds
        const delayedTime = Math.max(0, elapsedTime - enlargeConfig.delay);

        // Ripple effect phase (0 to rippleDuration)
        const rippleProgress = Math.min(
          1,
          elapsedTime / enlargeConfig.rippleDuration
        );

        // Scale animation phase (starts after delay)
        ...
        // Apply ripple effect with increased intensity
        const material = enlargingMesh.material as THREE.ShaderMaterial;
        if (material.uniforms) {
          // Intensify the ripple during click
          const rippleIntensity =
            enlargeConfig.rippleIntensity * (1 - rippleProgress);
          material.uniforms.uHover.value = Math.min(1, rippleIntensity);
          material.uniforms.uTime.value = shaderTime * 2; // Speed up ripple animation

          // Keep mouse UV at click point for centered ripple
          if (enlargingMesh.userData.clickUV) {
            material.uniforms.uMouse.value.copy(enlargingMesh.userData.clickUV);
          }
        }

        // Scale animation - enlarge the image
        if (delayedTime > 0) {
          const targetScale = enlargeConfig.finalScale;
          ...
          // Move image to center of screen and bring it forward
          enlargingMesh.position.z = easedScaleProgress * 5; // Bring forward
          enlargingMesh.position.x = 0; // Center horizontally
          enlargingMesh.position.y = -0.2; // Center vertically (adjust based on camera)
        ...
        if (scaleProgress >= 1) {
          console.log("Enlargement animation complete");
          // Animation complete - image is now full screen
          // Navigation logic will be handled by the setTimeout in onCanvasClick
        }
      }
      ...
      meshes.forEach(({ mesh }) => {
        // Skip normal animation for the enlarging mesh
